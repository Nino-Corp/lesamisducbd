 ...
 */
const mapPrestaProduct = (p, allCombinations = [], allOptionValues = []) => {
    const { url, key } = getPrestaConfig();
    ...
    // --- Build Variants ---
    let productVariants = [];
    if (p.associations && p.associations.combinations && allCombinations.length > 0 && allOptionValues.length > 0) {
        const combIds = p.associations.combinations.map(c => String(c.id));
        const pCombinations = allCombinations.filter(c => combIds.includes(String(c.id)));
        
        productVariants = pCombinations.map(c => {
            let weightLabel = '';
            if (c.associations && c.associations.product_option_values) {
                const optValId = String(c.associations.product_option_values[0].id);
                const optVal = allOptionValues.find(ov => String(ov.id) === optValId);
                if (optVal) {
                    weightLabel = typeof optVal.name === 'string' ? optVal.name : (optVal.name?.[0]?.value || '');
                }
            }

            const priceImpactHT = parseFloat(c.price || 0);
            const priceImpactTTC = Math.round(priceImpactHT * (1 + taxRate) * 100) / 100;
            const variantPriceTTC = Math.round((rawPrice + priceImpactHT) * (1 + taxRate) * 100) / 100;
            const formattedVariantPrice = variantPriceTTC.toLocaleString('fr-FR', { style: 'currency', currency: 'EUR' });

            return {
                id: String(c.id), // id_product_attribute
                label: weightLabel,
                priceImpactHT,
                priceImpactTTC,
                priceTTC: variantPriceTTC,
                formattedPrice: formattedVariantPrice,
                isDefault: c.default_on === '1' || c.default_on === 1
            };
        });

        // Sort variants so default is first, or sort by price
        productVariants.sort((a, b) => a.priceTTC - b.priceTTC);
    }
...
export const productServicePresta = {
    async getProducts() {
        ...
        // 2. Fetching specific prices (discounts by group), combinations, and option values
        let specificPrices = [];
        let combinations = [];
        let optionValues = [];
        
        try {
            console.log('[ProductService] Fetching specific prices, combinations, and option values...');
            const [specificRes, combRes, optRes] = await Promise.all([
                fetchPrestaApi('/specific_prices', { display: 'full' }),
                fetchPrestaApi('/combinations', { display: 'full' }),
                fetchPrestaApi('/product_option_values', { display: 'full' })
            ]);

            if (specificRes?.specific_prices) specificPrices = specificRes.specific_prices;
            if (combRes?.combinations) combinations = combRes.combinations;
            if (optRes?.product_option_values) optionValues = optRes.product_option_values;
            
            console.log(`[ProductService] Found ${specificPrices.length} specific price rules, ${combinations.length} combinations, ${optionValues.length} option values.`);
        } catch (e) {
            console.error("[ProductService] Could not fetch additional data (prices, combinations):", e.message);
        }

        // 3. Mapping products and injecting their specific prices and variants
        return active.map(p => {
            const productMapped = mapPrestaProduct(p, combinations, optionValues);
    ...
    async getProduct(slug) {
        ...
        // Fetch specific prices, combinations, and option values
        let specificPrices = [];
        let combinations = [];
        let optionValues = [];
        ...
        try {
            const [specificRes, combRes, optRes] = await Promise.all([
                fetchPrestaApi('/specific_prices', { display: 'full' }),
                fetchPrestaApi('/combinations', { display: 'full', 'filter[id_product]': productRow.id }),
                fetchPrestaApi('/product_option_values', { display: 'full' })
            ]);
        ...
        const productMapped = mapPrestaProduct(productRow, combinations, optionValues);
