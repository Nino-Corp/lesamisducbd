...
const LEGACY_META = {
    TransparenceHeader: { icon: '✨', label: 'En-tête Transparence' },
    TransparenceQuote:  { icon: '🗣️', label: 'Citation / Profil' },
    TransparenceFeature:{ icon: '⭐', label: 'Fonctionnalité' },
    TransparenceCertificates: { icon: '📄', label: 'Certificats' },
...
/* ── Main Component ──────────────────────────────── */
export default function TransparenceContentPage() {
    ...
    useEffect(() => {
        ...
        fetch('/api/admin/content/transparence', { signal: controller.signal })
            .then(r => r.json())
            .then(data => {
                if (!data.sections || data.sections.length === 0) {
                    ...
                    data.sections = [
                        { id: 'hero', type: 'TransparenceHeader', props: {
                            title: data?.hero?.title || "Transparence",
                            subtitle: data?.hero?.subtitle || "La qualité sans compromis."
                        }, isVisible: data?.hero?.isVisible !== false },
                        
                        { id: 'quote', type: 'TransparenceQuote', props: {
                            image: "/images/nelson.png",
                            text: data?.quote?.text || "Nous, Les Amis du CBD sommes de vrais passionnés du cannabis !\nOr nous étions extrêmement déçus de voir le marché français inondé par des produits de qualité médiocre, vendus bien souvent à des prix délirants !\nNous nous sommes donc mis au travail, et nous avons été surpris de voir ce qui était possible.",
                            author: data?.quote?.author || "Nelson - Les Amis du CBD"
                        }, isVisible: data?.quote?.isVisible !== false },
                        
                        { id: 'section1', type: 'TransparenceFeature', props: {
                            title: data?.section1?.title || "Meilleure qualité",
                            icon: "star",
                            col1: data?.section1?.col1 || {
                        ...
                        }, isVisible: data?.section1?.isVisible !== false },

                        { id: 'section2', type: 'TransparenceFeature', props: {
                            title: data?.section2?.title || "Moins cher",
                            icon: "badgeEuro",
                            col1: data?.section2?.col1 || {
                        ...
                        }, isVisible: data?.section2?.isVisible !== false },

                        { id: 'certificats', type: 'TransparenceCertificates', props: {
                        ...
                        }, isVisible: data?.certificatsVisible !== false }
                    ];
                }
    ...
    const save = async () => {
        setSaving(true);
        try {
            await fetch('/api/admin/content/transparence', {
                method: 'POST',
    ...
    return (
        <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#f1f5f9', overflow: 'hidden' }}>
            ...
            {!isFullscreen && (
                <header style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        ...
                        <div>
                            <h1 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>🔍 Transparence</h1>
                            <code style={{ fontSize: '0.72rem', color: '#94a3b8' }}>/transparence</code>
                        </div>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        ...
                        <a href="/transparence" target="_blank" rel="noopener noreferrer" style={{
                            padding: '8px 16px', borderRadius: '8px', background: '#e0f2fe', color: '#0369a1',
                            textDecoration: 'none', fontWeight: 600, fontSize: '0.85rem',
                        }}>👁 Aperçu</a>

                        <a href="https://www.lesamisducbd.fr/transparence" target="_blank" rel="noopener noreferrer" style={{
            ...
            {showSEOModal && (
                <SEOPanel
                    page={{ slug: '', title: 'Transparence', seo: meta.seo || {}, status: 'published' }}
                    sections={previewSections}
                    onUpdate={(patch) => {
